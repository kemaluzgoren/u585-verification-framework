...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* Unreachable: tx_kernel_enter() inside MX_ThreadX_Init() above never
     * returns. Sensor polling now runs in the "System Thread" started by
...
/* Routes printf()/_write() (Core/Src/syscalls.c) to USART1, the ST-Link
 * Virtual COM Port (T.VCP_TX/PA9, T.VCP_RX/PA10) - open a serial terminal
 * on that COM port at 115200 8N1 to see it. Without this, __io_putchar
 * stays an unresolved weak symbol and every printf() call in the project
 * (including netx_main_thread_entry's IP address log) faults instead of
 * printing anything. */
