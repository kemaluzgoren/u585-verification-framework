 ...
 *          -> tx_api.h/nx_api.h (fine - resolves to this test's real
 *          Middlewares tx_api.h and this directory's fake nx_api.h, see
 *          this directory's CMakeLists.txt) but also ->
 *          mx_wifi_conf_template.h -> main.h (STM32 HAL, not
 *          host-compilable) plus the whole MX_WIFIObject_t API surface
 *          mx_wifi_azure_rtos.c never touches - it only needs
 *          MX_ASSERT/MX_STAT/MX_STAT_LOG and the prototypes of the
 *          functions it defines (normally declared by
